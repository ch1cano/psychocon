...
const Form = () => {
  ...
  const sendEmail = (e) => {
    emailjs.sendForm("service_1", "template_wv4nz5h", formRef.current, "LusvWydXUgt_5TJIF")
      .then(result => {
        console.log(result)
        reset();
        setIsSubmit(true)
    ...
    <div className={s.formContiner}>
      <div className={s.descriptions}>
        <div className={s.divBunner}>
          <h2 className={s.buner}>Запись онлайн</h2>
        </div>
        <div className={s.divBunerDesc}>
          <p className={s.bunerDesc}>Администратор центра сориентирует вас по специфике услуг и врачей и запишет к нужному специалисту.</p>
        </div>
      </div>
      {isSubmit && (
      ...
      <form ref={formRef} className={s.form} onSubmit={handleSubmit(sendEmail)}>
        <div className={s.formMain}>
          ...
          <select className={s.select} name='usluga' {...register('usluga', { required: 'Выберите услугу' })}>
            <option value='' hidden>
              -- Выберите услугу --
            </option>
          ...
          <br />
          {errors.usluga && <span className={s.errorsSpan}>{errors.usluga.message}</span>}
          ...
          />
          <br />
          {errors.data && <span className={s.errorsSpan}>{errors.data.message}</span>}
          <div className={s.descriptionInput}>Специалист</div>
          <select className={s.select} name='specialist' {...register('specialist', { required: 'Выберите специалиста' })}>
            <option value='' hidden>
          ...
          />
          <br />
          {errors.name && <span className={s.errorsSpan}>{errors.name.message}</span>}
          ...
          <Controller
            ...
            render={({ field }) => (
              <Input {...field} country='RU' international withCountryCallingCode defaultCountry='RU' onChange={(value) => field.onChange(value)} />
            )}
          />
          <br />
          {errors.tel && <span className={s.errorsSpan}>{errors.tel.message}</span>}
          <div className={s.descriptionInput}>Комментарий к записи &#40;не обязательно&#41;</div>
          <input name='comment' className={s.formInput} type='text' {...register('comment')} />
        </div>
